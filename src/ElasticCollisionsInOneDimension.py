...
class ElasticCollisionsInOneDimension:
    def __init__(self, width, height, parentApp):
        self.parentApp = parentApp
        self.width = width  # self.width of the self.parent_app.getScreen()
        self.height = height  # self.height of the self.parent_app.getScreen()
    ...
    def getScreen(self):
        return self.parentApp.get_screen()
    ...
    def showText(self, text, centreX, centreY, fontColour, fontBg, fontSize):
        ...
        self.parentApp.get_screen().blit(text, textrect)  # Shows text on self.parent_app.getScreen()

    def updateSliders(self):
        for slider in self.listOfSliders + self.selectedParticle.sliders:
            if slider.move_pointer:
                if (slider.x < pygame.mouse.get_pos()[0] < slider.x + slider.width):
                    ...
                    slider.setVariable()
            if slider in self.selectedParticle.sliders:
                self.selectedParticle.sliders[0].set_value(self.selectedParticle.mass)
                self.selectedParticle.sliders[1].set_value(self.selectedParticle.velocity)
            slider.draw_slider()
    ...
    def playSimulation(self):
        ...
        try:
            filePath = IMAGES_PATH + '\Pause' + '.jpg'
            self.pausePlayButton.image = pygame.image.load(filePath).convert()
        except:
        ...
        for event in self.parentApp.events:
            ...
            if (event.type == pygame.MOUSEBUTTONDOWN):
                if (event.button == 1):
                    ...
                    if (self.pausePlayButton.mouse_over_button()):
                        if (self.run):
                            self.pauseSimulation()
                    ...
                    elif (self.addParticleButton.mouse_over_button()):
                    ...
                    elif (self.resetButton.mouse_over_button()):
                        self.reset = True

                    elif (self.settingsButton.mouse_over_button()):
                        self.settingsWindow()
                    ...
                    for slider in self.listOfSliders + self.selectedParticle.sliders:
                        if (slider.pointer.mouse_over_point()):  # Slider control of user
    ...
    def updateParticles(self):
        for i in range(0, len(self.listOfParticles)):
            ...
            particle.draw()  # Draws the circles on the self.parent_app.getScreen()
    ...
    def deleteAllWindows(self):
        for particle in self.listOfParticles:
            try:
                particle.sliders[0].root.destroy()
            except:
                pass
            try:
                particle.sliders[1].root.destroy()
            except:
                pass
            try:
    ...
    def updateWindow(self):
            ...
            for particle in self.listOfParticles:
                try:
                    ...
                    if (i == 4):
                        ...
                        pygame.draw.aaline(self.parentApp.get_screen(), (255, 0, 0),
        ...
        try:
            ...
            pass
        self.pausePlayButton.show_button()
        self.addParticleButton.show_button()
        self.resetButton.show_button()
        self.settingsButton.show_button()

        # Draws the surface on which the particles are moving
        pygame.draw.line(self.parentApp.get_screen(), (0, 0, 0), (0, self.yCor),
                         (self.width, self.yCor))
        # pygame.display.update() # Updates the self.parent_app.getScreen()
    ...
    def main(self):
        # Fills the self.parent_app.getScreen() with white to erase the current object
        self.parentApp.get_screen().fill(self.bgColour)
        if self.reset:
