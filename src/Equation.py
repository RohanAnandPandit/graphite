...
class Equation:  # Used for plotting graphs
    def __init__(self, parentWindow, equationType):
        ...
        self.sliders = [Slider('a', 'a', 0, self.parentWindow.height - 60,
    ...
    def draw_graph(self):
        if self.type == 'parametric':
            ...
            while i < len(self.listOfPoints2) - add:
                ...
                if (self.listOfPoints2[i] != '' and
                        self.listOfPoints2[i - add] != ''):
                    if (self.listOfPoints2[i].visible() or
                            self.listOfPoints2[i - add].visible()):
                        plot = True

                    elif (self.listOfPoints2[i].screen_pos[0] < 0
                          and self.listOfPoints2[i - add].screen_pos[0] < 0
                          or self.listOfPoints2[i].screen_pos[0] > self.parentWindow.width
                          and self.listOfPoints2[i - add].screen_pos[0] > self.parentWindow.width):
                        plot = False

                    elif (self.listOfPoints2[i].screen_pos[1] < 0
                          and self.listOfPoints2[i - add].screen_pos[1] < 0
                          or self.listOfPoints2[i].screen_pos[1] > self.parentWindow.height
                          and self.listOfPoints2[i - add].screen_pos[1] > self.parentWindow.height):
                        plot = False
                    ...
                    if plot:
                        ...
                        if self.thickness == 1:
                            ...
                            pygame.draw.aaline(self.parentWindow.get_screen(),
                                               self.colour,
                                               self.listOfPoints2[i - add].screen_pos,
                                               self.listOfPoints2[i].screen_pos)
    ...
    def mouse_over_graph(self):
        ...
        for point in self.points:
            if point == '' or point.screen_pos == None:
                continue
            ...
            while i < len(self.points) - 1:  # Iterates through all the points
                if (self.points[i] != '' and
                    ...
                    try:
                        ...
                        if (min(point1.screen_pos[0], point2.screen_pos[0]) < pygame.mouse.get_pos()[0]
                                and pygame.mouse.get_pos()[0] < max(point1.screen_pos[0], point2.screen_pos[0])
    ...
    def window(self, x=None, y=None):  # Equationwindow
        ...
        if (self.type == 'parametric'):
            self.parentWindow.parametric_equation_windows.append(self)
            ...
            self.parentWindow.artesian_equations_windows.append(self)
    ...
    def convert(self):
        self.parentWindow.equations.remove(self)  # Removes any record of the equation
        for point in self.points:  # Iterates throiugh list of points
            point.equation = None  # Sets equation to None
            self.parentWindow.points.append(point)  # Adds point to points
            if self in self.parentWindow.selected_equations:  # If the equation was selected,
                self.parentWindow.selected_points.append(point)  # so will the points

        if self in self.parentWindow.selected_equations:
            self.parentWindow.selected_equations.remove(self)  # Removes any record of the equation
    ...
    def sliderSettings(self):
        if not self.showSliders:
            for equation in self.parentWindow.equations:
    ...
    def deleteEquation(self):
        if self in self.parentWindow.equations:
            self.parentWindow.equations.remove(self)

        if self in self.parentWindow.selected_equations:
            self.parentWindow.selected_equations.remove(self)
    ...
    def calculatePoints(self):
        ...
        # right data type
        startValue = eval(syntax_correction(self.startValue))
        endValue = eval(syntax_correction(self.endValue))
        xEquation = syntax_correction(self.xEquation)
        yEquation = syntax_correction(self.yEquation)
        zEquation = syntax_correction(self.zEquation)
        accuracy = eval(self.accuracy)

        # Removes all the older points before the new points are calculated
        for point in self.points:
            if point in self.parentWindow.points:
                self.parentWindow.points.remove(point)

        self.points = []  # Initialises new list
        self.listOfPoints2 = []  # Initialises new list
        try:
            t = self.sliders[len(self.sliders) - 2].variable
            self.limit1Point2.set_cor(t, eval(syntax_correction(self.yEquation)), 0)
            t = self.sliders[len(self.sliders) - 1].variable
            self.limit2Point2.set_cor(t, eval(syntax_correction(self.yEquation)), 0)
        except:
            pass
        ...
        if self.type == 'parametric':
            ...
            while t <= endValue:
                try:
                    ...
                    if not self.points:
                        ...
                        self.listOfPoints2.append('')
                    elif self.listOfPoints2[len(self.points) - 1] != '':
                        self.points.append('')
            ...
            if True:
                ...
                # of the same name
                startX = eval(syntax_correction(self.startX))
                endX = eval(syntax_correction(self.endX))
                startY = eval(syntax_correction(self.startY))
                endY = eval(syntax_correction(self.endY))
                startZ = eval(syntax_correction(self.startZ))
                endZ = eval(syntax_correction(self.endZ))
                accuracy = eval(self.accuracy)
                ...
                while x <= endX:
                    ...
                    while y <= endY:
                        ...
                        while z <= endZ:
                            try:
                                equation = list(syntax_correction(self.cartesianEquation))
                                l = len(equation)
                                i = 0
                                while i < l:
                                    if equation[i] == '=':
                ...
                self.parentWindow.get_screen().fill((255, 255, 255))
    ...
    def setInfoParametric(self, xEquation, yEquation, zEquation, startValue,
        ...
        if self not in self.parentWindow.equations:
            self.parentWindow.equations.append(self)
        try:
            t = self.limit1
            self.limit1Point1.set_cor(self.limit1, 0, 0)
            self.limit1Point2.set_cor(self.limit1,
                                      eval(syntax_correction(self.yEquation)), 0)
            t = self.limit2
            self.limit2Point1.set_cor(self.limit2, 0, 0)
            self.limit2Point2.set_cor(self.limit2,
                                      eval(syntax_correction(self.yEquation)), 0)
        except:
            pass
    ...
    def setInfoCartesian(self, cartesian_equation, startX, endX, startY, endY,
        ...
        c = self.c
        d = self.d
        self.cartesianEquation = cartesian_equation  # Expression to calculate points
        ...
        self.sliderSettings()
        if self not in self.parentWindow.equations:  # Adds Equation object to list of equations
            self.parentWindow.equations.append(self)
