...
from maths.Matrices import matrix_multiply
...
class ElasticCollisionsInTwoDimensions:
        ...
        for slider in self.listOfSliders + self.selectedParticle.sliders:
            if (slider.move_pointer):
                if (slider.x < pygame.mouse.get_pos()[0] < slider.x + slider.width):
                    ...
                    slider.pointer.screen_pos = (pygame.mouse.get_pos()[0],
                                                 slider.pointer.y)
                    slider.setVariable()
        ...
        for i in range(0, 5):
            for particle in self.listOfParticles:
                try:
                    ...
                    if (i == 4):
                        pygame.draw.aaline(self.parentWindow.get_screen(), (255, 0, 0),
    ...
    def getComponentVelocities(self, p1, p2):
        ...
        v1 = matrix_multiply(matrix, [[p1.velocity[0]], [p1.velocity[1]], [0], [1]])
        v2 = matrix_multiply(matrix, [[p2.velocity[0]], [p2.velocity[1]], [0], [1]])
    ...
    def collisionCalculator2D(self, particle1, particle2):
        ...
        pygame.draw.aaline(self.parentWindow.get_screen(), (255, 0, 0), (p1.x + 3 * dx, p1.y - 3 * dy),
                           (p2.x - 3 * dx, p2.y + 3 * dy))
        pygame.draw.circle(self.parentWindow.get_screen(), (0, 255, 0), (int(p1.x), int(p1.y)),
                           p1.radius + 5, 2)
        pygame.draw.circle(self.parentWindow.get_screen(), (0, 0, 255), (int(p2.x), int(p2.y)),
                           p2.radius + 5, 2)
