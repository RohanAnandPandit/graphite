...
def derivative(equation, x):
  def f(x):
    t = x
    r = x
    return eval(syntaxCorrection(equation))
  h = 0.0000000001
  return (f(x + h) - f(x)) / h

def newtonRhapson(equation, x0, iterations = 20):
  x = x0
  t = x0
  r = x0
  equation = syntaxCorrection(equation)
...
def regression(listOfPoints):
    ...
    for point in listOfPoints:
        xSum += point[0]
    xMean = xSum / len(listOfPoints)

    x2Sum = 0
    for point in listOfPoints:
        x2Sum += point[0] ** 2
    x2Mean = x2Sum / len(listOfPoints)

    ySum = 0
    for point in listOfPoints:
        ySum += point[1]
    yMean = ySum / len(listOfPoints)
