...
def syntaxCorrection(equation, replaceFunctions = True):
    ...
    while (i < l):
        ...
        if (i < l - 1):
            ...
            if (char not in superScript and equation[i+1] == '⁽' ):
                equation[i+1] = '**('

        if (char in operators+constants+superScript):
            try:
                equation[i] = key[char]
            except:
                pass
...
def entryFormatter(entry):
    l = len(entry.get())
    i = 0
    while (i < l):
        if (i != 0):
            # If the character before the current character is in superScript
            # then the current character should also be in superScript
            if (entry.get()[i-1] in superScript  and entry.get()[i] not in superScript):
                s = entry.get()[i]
                entry.delete(i)
                entry.insert(i,convertToSuperScript[s])

            # If the user wants to raise something to a power then it will be
            # replaced by super script open bracket
            if (entry.get()[i] == '^' and entry.get()[i-1] not in superScript):
                entry.delete(i,i+1)
                entry.insert(i,'⁽')
                entry.icursor(i+2)
...
def substituteValues(obj, equation):
    i = 0
    equation = list(equation)
    l = len(equation)
    while (i < l):
        ...
        if (char in ['a','b','c','d']):
            ...
            try:
                if (key[char] in ['a','b','c','d']):
                    equatoin[i] =  str(round(eval('obj.' + key[char]),3))
            except:
                pass
            try:
